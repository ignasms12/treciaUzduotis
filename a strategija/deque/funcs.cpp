...
void splitOutput(std::deque<Studentas> &vektoriux, std::deque<Studentas> &kietekai, std::deque<Studentas> &vargsiukai){
	int count = 0;
	int kitasCount = 0;
	for(int k=0; k<vektoriux.size(); k++){
		...
		else if(vektoriux[k].bendras_pazymys < 5){
...
void print(std::deque<Studentas> printeris, std::deque<Studentas> vargsiukai, std::deque<Studentas> kietekai, int pasirinkimas){
	std::cout << std::endl;
	int hehehey;
	...
	if(hehehey == 0){
		...
		for(int sup=0; sup < kietekai.size(); sup++) { 
			std::cout << std::endl;
			std::cout.width(15);
			std::cout << std::left << kietekai[sup].name;
			std::cout.width(20);
			std::cout << std::left << kietekai[sup].surname;
			std::cout << std::fixed << std::setprecision(2) << kietekai[sup].bendras_pazymys << std::endl;
	}
...
void outputToFile(std::deque<Studentas> kiec,std::deque<Studentas> vargs, int pasirinkimas){
...
void readFile(std::deque<Studentas> &printeris, int k, int pasirinkimas){
...
void ivedimoPorankis(std::deque<Studentas> &printeris, int &pasirinkimas, int k, int ivedimoPorank, char option){
